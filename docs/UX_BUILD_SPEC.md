...
The post-CR4 Figma Make catch-up tranche uses the supplied Figma Make designs as visual and interaction intent, but production-intended routes must remain backend-backed. `/aura-note` now uses a Figma-derived shell pattern with sidebar navigation, current user context, badges, notifications, activity, dashboard metrics, and disabled-feature states from `GET /api/v1/app-shell`. Sidebar collapse and notification drawer open/closed state are transient local React state. Route counts, role states, badges, safety posture, and disabled-feature posture are API-backed. `/aura-note/workspace/[appointmentId]` now loads the editor document from `GET /api/v1/notes/{noteId}/content` and uses typed autosave/version APIs for persisted content, revisions, sections, and restore. Unsaved editor typing is transient form state only until autosave. `/aura-note/schedule` now uses backend-backed schedule filters, location/room metadata, virtual visit status, workspace validation, and metadata-only chart-intake status from typed schedule APIs; local filter controls are transient only. The workspace now also renders API-backed transcript live state from `GET /api/v1/documentation-workspace/appointments/{appointmentId}/transcript/live`, suggestion education/evidence and human-review-required labels, selected-code disposition/removal/category-change state, and compliance issue action history with acknowledge/resolve actions. `/aura-note/finalization/[noteId]` now renders Design 2 finalization runtime panels from API-backed `FinalizationSessionDto`: evidence spans, selected-code/item status, dual editor variants, patient questions, care-plan candidates, patient insight metadata, billing validation, and dispatch metadata. `/aura-note/operations` now renders Design 1 operations analytics, activity, notifications, and settings-runtime summary from `GET /api/v1/standalone/operations/runtime`; local state remains limited to tabs, in-progress form values, and button status. The Figma Make source/node visual pass now adds the Design 1 quick-action command band, command dashboard, workspace editor command deck, selected-code rail, suggestion intelligence rail, analytics tab set, KPI/chart stage, settings governance tabs, and the Design 2 evidence highlighter, patient-question popup, and billing/dispatch dock. Live chart upload, live patient portal delivery, live EHR completeness/writeback, live audio streaming, raw PHI audio transport, production analytics vendors, patient-facing revenue, live claim submission, and autonomous clinical/coding/billing behavior remain disabled/gated. Formal screenshot-baseline approval and founder/designer production visual signoff remain separate evidence before launch-candidate claims.
