...
These are partial inputs only. `WO-078` through `WO-080` remain `planned` until the remaining AURA Note-specific configuration, approval, credential-delivery, scope, and operational details are documented. For `WO-081`, the remaining gap has narrowed to implementation evidence: actual Azure provisioning, managed identity resource IDs, private endpoint/private DNS/firewall evidence, role assignments, approved secret/config-store references, and synthetic no-PHI live-readiness tests.
