...
The founder/operator supplied these partial inputs on 2026-06-02 and 2026-06-03:
...
- `WO-078`: launch scope is internal only; launch tenant/site scope is Azure tenant scope only with no site-specific scope approved; rollback procedure owner, incident commander, and launch communications owner are the founder/operator; support hours/escalation path and access review cadence are none for internal-only planning; no expanded role set is approved because the founder answered `No` for roles enabled at launch.
...
- `WO-081`: production Azure storage planning should evaluate Flow's Key Vault, Blob soft-delete/versioning, and recovery posture, while deciding separate AURA Note PHI artifact-storage, deletion, restore, and monitoring controls; the same verified Azure baseline is available for storage planning. The founder/operator has now confirmed `eastus` and authorized Codex to make the remaining non-secret choices. `docs/PRODUCTION_AZURE_STORAGE_DECISION_RECORD.md` selects candidate account `auranoteeastus91d0`, artifact containers, managed-identity access, private endpoint requirement, server-mediated download TTLs, raw-audio/transcript retention, soft-delete/versioning/legal-hold posture, restore-readiness cadence, and monitoring requirements.

These are partial inputs only. `WO-078` through `WO-080` remain `planned` until the remaining AURA Note-specific configuration, approval, credential-delivery, scope, and operational details are documented. For `WO-078`, the remaining launch-governance blockers are exact enabled roles or explicit no-role observer posture, disabled-feature inventory acceptance, go/no-go criteria, backup/restore drill acceptance criteria, backup incident commander or explicit no-backup risk acceptance, and explicit written approval if any launch flag is intended to change. For `WO-081`, no-PHI Azure infrastructure has now been provisioned and recorded in `docs/PRODUCTION_AZURE_STORAGE_PROVISIONING_EVIDENCE.md`; the remaining gap has narrowed to approved runtime config/secret-store references, app private-network integration, synthetic no-PHI object-level live-readiness tests, legal-hold/deletion/restore-readiness execution evidence, monitoring/alert wiring, and explicit production flag approval.
