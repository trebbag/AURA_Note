...
`WO-063` adds the CR-1 identity runtime boundary. Non-health API routes now require explicit `AURA_NOTE_AUTH_MODE`; local synthetic/demo headers are trusted only under `AURA_NOTE_AUTH_MODE=local_demo` or `AURA_NOTE_AUTH_MODE=local_synthetic`. Local demo mode labels and normalizes synthetic headers for browser/dev evidence, while strict local synthetic mode requires role, user, session, and purpose context. Preview/production OIDC, production SAML, and ClinicOS delegated auth postures fail closed before controller execution because no live adapter, credential, token parser, SAML assertion handling, or ClinicOS delegated identity contract is configured. The boundary produces PHI-safe error envelopes, response posture labels, and audit-safe structured log metadata for identity accepted, denied, expired-session, disabled-user, missing-purpose, wrong-tenant/site, wrong-purpose, synthetic-header-forbidden, and delegated-not-configured states. This remains synthetic/local identity evidence only; production SSO, MFA, SCIM, account recovery, break-glass, live ClinicOS delegation, production PHI access, claim submission, and launch behavior remain disabled.
