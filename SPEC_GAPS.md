...
- **Partial Azure baseline captured 2026-06-02:** tenant `b9b1d566-d7ed-44a4-b3cc-cf8786d6a6ed`, subscription `Subscription Malady` (`91d0e7fe-e9c6-40a0-af0f-98a9dc07b218`), resource group `AURA_resource_group`, location `eastus`, provisioning state `Succeeded`.
- **Non-secret decisions captured 2026-06-02:** `eastus` accepted; candidate storage account `auranoteeastus91d0`; `StorageV2`; `Standard ZRS`; managed identity; candidate managed identity `aura-note-storage-mi`; candidate Key Vault reference `aura-note-kv-91d0`; artifact containers; private endpoint requirement; server-mediated download TTLs; raw-audio 7-day purge eligibility; transcript indefinite retention; 14-day Blob/container soft delete; Blob versioning; legal-hold deletion blocking; quarterly synthetic restore-readiness; 7-year evidence/audit export retention default; and monitoring/alerting requirements are recorded in `docs/PRODUCTION_AZURE_STORAGE_DECISION_RECORD.md`.
- **Decision or evidence still needed before live use:** actual Azure provisioning, actual managed identity client/resource IDs, private endpoint/private DNS/firewall evidence, least-privilege role assignments, approved secret/config-store references, synthetic no-PHI live-readiness tests, and explicit production flag approval before PHI-bearing object delivery, destructive production deletion, PHI restore execution, or production launch.
